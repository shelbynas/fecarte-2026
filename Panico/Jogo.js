...
// ===================================================================
// NOITE DE PÂNICO — história de terror slasher, protagonista único (Bia).
// Cada cena pode ter:
...
const story = {
    start: {
        ...
        mood: "calm",
        options: [
            { text: "Continuar", type: "continue", next: "casaSozinha" }
        ]
    },
    ...
    garagem: {
        speaker: "",
        text: "Na garagem escura, você encontra o velho taco de beisebol do seu pai encostado na parede — e, pendurada no gancho ao lado, a chave do carro.",
        mood: "calm",
        icon: "bat",
        addToInventory: ["Taco de Beisebol", "Chave do Carro"],
        options: [{ text: "Voltar", type: "continue", next: "casaSozinha" }]
    },
    ...
    banheiro: {
        speaker: "",
        text: "Você abre o armarinho para pegar o kit de primeiros socorros e, no espelho atrás da porta, por um instante, jura ver um vulto atrás de você. Quando se vira, não há nada. Só o coração acelerado.",
        mood: "tense",
        event: "danger",
        icon: "bandage",
    ...
    chegamCasaRafaDeCarro: {
        speaker: "",
        text: "Vocês pegam o carro e chegam em poucos minutos, sãs e salvas, direto à porta da casa do Rafa.",
        mood: "tense",
        icon: "carkey",
    ...
    decisaoFugaCarro: {
        speaker: "DUDA",
        text: "Duda te encara, incrédula. \"Tem certeza? E o Rafa?\"",
        mood: "tense",
        options: [
...
// Tempo que o resultado do dado fica visível na tela antes de seguir a história.
const ROLL_DISPLAY_MS = 3200;
...
function anyOverlayOpen() {
    return !itensOverlay.classList.contains('hidden') || !endingsOverlay.classList.contains('hidden');
}
...
// Mostra o retrato da Duda (sprite real) só quando ela está presente na
// cena; caso contrário, o espaço fica vazio (sem ícones ilustrativos).
function updatePortrait(sceneKey) {
    const emotion = DUDA_EMOTIONS[sceneKey];
    const imageSrc = emotion ? DUDA_IMAGES[emotion] : null;
...
function onDialogueReady(sceneKey, scene) {
    ...
    if (scene.isEnding) {
        const isNew = unlockEnding(sceneKey);
        dadoResultado.style.display = 'none'; // por segurança, caso algo tenha ficado preso
    }
}
...
function handleOption(option) {
    ...
    if (option.roll) {
        ...
        dadoResultado.innerHTML =
            ` Teste de sobrevivência<br>` +
            `<span style="font-size:1.4em;">${roll}</span> / precisa de ${option.successThreshold}+<br>` +
            (success ? 'SUCESSO' : 'FALHA');

        setTimeout(() => {
            dadoResultado.style.display = 'none';
            isProcessingOption = false;
            showScene(success ? option.successNext : option.failNext);
        }, ROLL_DISPLAY_MS);
    } else if (option.next) {
        isProcessingOption = false;
